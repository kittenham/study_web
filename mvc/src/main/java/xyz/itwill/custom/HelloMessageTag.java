...
//태그 속성은 있지만 태그 내용이 없는 커스텀 태그 클래스
public class HelloMessageTag extends TagSupport{
	...
	//생성자에게 객체 생성에 필요한 초기화 작업 관련
	public HelloMessageTag() {
		//커스텀 태그 사용시 속성을 생략할 경우 기본적으로 사용될 속성값으로 필드에 저장
		//=> 커스텀 태그의 속성이 필수인 경우 필드 기본값 설정 생략한다.
		//	(한마디로 속성이나 속성값 둘 중 하나를 적으면 나머지 하나는 생략가능하다는 말)
		name="홍길동";
	}
	...
	//커스텀 태그사용시 태그 속성을 사용하여 속성값을 설정할 경우 Setter 메소드 자동 호출
