...
//1. 클라이언트의 모든 요청을 받을 수 있도록 서블릿을 설정하여 단일 진입점의 기능을 구현한다.
...
//@WebServlet("/*.do") => servelt이 많을때는 얘를 사용하는게 좋지만 지금은 컨트롤러 하나만 필요하므로 [web.xml]을 사용하는 것이 더 효율적이다.
public class ControllerServlet extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//2. 클라이언트의 요청을 분석한다. : 요청 URL 주소를 이용해 클라이언트가 무엇을 원하는지 알 수 있다.
		//   -- http://localhost:8000/mvc/XXX.do
		//HttpServletRequest.getRequestURI() : 요청 URL 주소에서 URI 주소를 반환하는 메소드
		String requestURI=request.getRequestURI();
		//System.out.println("requestURI = "+requestURI); //출력 : requestURI = /mvc/XXX.do
		
		//HttpServletRequest.getContextPath() : 요청 URL 주소에서 컨텍스트가 저장된 경로를 반환하는 메소드
		String contextPath=request.getContextPath(); //출력 : contextPath = /mvc
		//System.out.println("contextPath = "+contextPath);//contextPath = /mvc

		
		String command=requestURI.substring(contextPath.length());
		System.out.println("command = "+command); //출력 : command = /XXX.do
		...
		//=> 인터페이스로 생성된 참조변수에는 인터페이스를 상속받은 모든 자식클래스(모델)의 객체를 생성할 수 있다.
		Action action = null;
		
		//클라이언트 요청 정보를 구분하여 인터페이스 참조변수에 필요한 모델 역할의 클래스로 객체를 생성해 저장한다.
