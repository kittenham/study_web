<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%-- 클라이언트가 보내온 쿠키값을 반환받아 클라이언트에게 전달하여 응답하는 JSP 문서 --%>
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
...
<body>
	...
	<%
		...
		for(Cookie cookie : cookies){
			//만약 Cookie 객체에 저장된 쿠키명을 반환받아 문자열과 비교하여 같은 경우에,
			if(cookie.getName().equals("userName"));{
				userName=cookie.getValue(); //Cookie 객체에 저장된 쿠키값을 반환받아 저장할 것이다.
			}
		}
	%>
	...
	value 표현식으로 쿠키값 출력 --%>
	<%-- => 쿠키객체는 객체이지 값이 아님. 값은 .value로 얻을 수 있다. --%>	
	<%-- 
	${cookie.userName } => 쿠키 객체
	${cookie.userName.value } => 쿠키값
	--%>
