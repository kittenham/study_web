...
<body>
	...
	document.getElementById("getBtn").onclick=function(){
		...
		//XMLHttpRequest 객체의 준비상태가 변경될 때마다 호출될 이벤트 처리 함수 등록(전역변수를 만들지는 않았음. 익명 함수를 등록함.)
		xhr.onreadystatechange=function(){
			if(xhr.readyState==4){ //XMLHttpRequest객체에 응답결과가 전달된 경우
				if(xhr.status==200){ //정상적인 응답결과를 받았다면(상태코드가 200 일때)
					//XMLHttpRequest 객체의 응답결과(HTML)를 이용하여 페이지의 태그 변경
					document.getElementById("display").innerHTML=xhr.responseText;
				} else { //정상적인 응답결과를 받지 못했다면
