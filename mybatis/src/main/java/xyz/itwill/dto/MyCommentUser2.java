...
public class MyCommentUser2 {
	...
	//값을 저장하는게 아니라 객체를 필드에 저장하는 것이다.
	//=> 따라서 MyComment1 클래스의 객체들을 comment라는 변수(필드)에 저장하기 때문에, 
	//	 필드를 사용해 다른 클래스를 만들면 Mycomment1 클래스의 객체가 변경되더라도 
	//	 따로 그를 포함하는 다른 클래스를 고칠 필요가 없기에 유지보수가 수월하다.
	private MyComment1 comment;
		
	//MYUSER 테이블(회원정보)의 컬럼값(객체)을 저장하기 위한 필드(값을 저장하는 것이 아님!) -> 검색행 1개
	private MyUser user;
