...
<%
	...
	if(member==null || member.getMemberStatus()==0 || !member.getPasswd().equals(passwd)) {//로그인 실패 
		...
		response.sendRedirect(request.getContextPath()
			+"/shoppingmall/index.jsp?group=member&worker=member_login&returnUrl="+URLEncoder.encode(returnUrl, "utf-8"));
		return;
	}
	...
	if(returnUrl.equals("")) {//반환받은 요청 JSP 문서의 URL 주소가 없는 경우 - 메인페이지 이동
		response.sendRedirect(request.getContextPath()+"/shoppingmall/index.jsp?group=main&worker=main_page");
