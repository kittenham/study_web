...
<div id="review_list">
	...
	<div style="text-align: right;">
		<button type="button" onclick="location.href='<%=request.getContextPath()%>/shoppingmall/index.jsp?group=review&worker=review_write';">글쓰기</button>
	</div>
	<% } %>
			...
			<tr>
				...
				<td class="subject">
					...
					<% if(review.getStatus()==1) {//일반 게시글인 경우 %>
						<a href="<%=request.getContextPath()%>/shoppingmall/index.jsp?group=review&worker=review_detail&num=<%=review.getNum()%>&pageNum=<%=pageNum%>&search=<%=search%>&keyword=<%=keyword%>"><%=review.getSubject()%></a>					
