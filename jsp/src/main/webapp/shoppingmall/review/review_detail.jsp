...
<%-- => [글변경] 태그를 클릭한 경우 [/shoppingmall/review/review_modify.jsp] 문서 요청 - 글번호, 페이지번호, 검색대상, 검색단어 전달 --%>
<%-- => [글삭제] 태그를 클릭한 경우 [/shoppingmall/review/review_remove_action.jsp] 문서 요청 - 글번호 전달 --%>
<%-- => [답글쓰기] 태그를 클릭한 경우 [/shoppingmall/review/review_write.jsp] 문서 요청 - 답글번호, 답글순서, 답글깊이, 페이지번호 전달 --%>
<%-- => [글목록] 태그를 클릭한 경우 [/shoppingmall/review/review_list.jsp] 문서 요청 - 페이지번호, 검색대상, 검색단어 전달 --%>
		...
		<tr>
			...
			<td class="content">
				...
				<% if(review.getReviewimg()!=null) { %>
					<img src="<%=request.getContextPath()%>/shoppingmall/review_images/<%=review.getReviewimg()%>" width="200">
				<% } %>	
			</td>
		</tr>		
	</table>
...
$("#modifyBtn").click(function() {
	location.href="<%=request.getContextPath()%>/shoppingmall/index.jsp?group=review&worker=review_modify"
			+"&num=<%=review.getNum()%>&pageNum=<%=pageNum%>&search=<%=search%>&keyword=<%=keyword%>";
});

$("#removeBtn").click(function() {
	if(confirm("게시글을 삭제 하시겠습니까?")) {
		location.href="<%=request.getContextPath()%>/shoppingmall/review/review_remove_action.jsp?num=<%=review.getNum()%>";
	}
});

$("#replyBtn").click(function() {
	location.href="<%=request.getContextPath()%>/shoppingmall/index.jsp?group=review&worker=review_write"
		+"&ref=<%=review.getRef()%>&restep=<%=review.getRestep()%>&relevel=<%=review.getRelevel()%>&pageNum=<%=pageNum%>";
});

$("#listBtn").click(function() {
	location.href="<%=request.getContextPath()%>/shoppingmall/index.jsp?group=review&worker=review_list"
