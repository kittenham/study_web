...
<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%-- 인증정보(아이디와 비밀번호)를 전달받아 인증처리하기 위한 JSP 문서 --%>    
<%--  => [login_form.jsp] 문서의 form 태그를 이용하여 POST 방식으로 요청하는 JSP 문서 --%>
<%-- 인증 성공 : 전달받은 아이디와 비밀번호가 올바른 경우 --%>
<%--  => 권한 관련 정보가 저장된 객체를 세션에 저장 -> 클라이언트에게 권한을 제공해준다 (= 로그인)  --%>
...
<%
	//request.getMethod() : JSP 문서를 요청한 요청방식(GET or POST)를 반환하는 메소드
	if(request.getMethod().equals("GET")){ //JSP 문서를 GET 방식으로 요청한 경우 - 비정상적인 
		/*
		//response.sendError(int sc) : 클라이언트에게 에러코드(4XX or 5XX)를 전달하는 메소드
		response.sendError(HttpServletResponse.SC_METHOD_NOT_ALLOWED); //405 오류코드
		return;
		*/
	...
	//request.setCharacterEncoding(String enc) : POST 방식으로 요청하여 리퀘스트 메세지 몸체부에 저장되어
	//전달되는 값에 대한 문자형태를 변경하는 메소드(POST방식일때만! GET방식에는 안해도됨)
	//request.setCharacterEncoding("utf-8"); //한글 전달값이 없는 경우 생략 가능

	//POST 방식으로 정상적으로 전달하면 전달값을 반환받아 저장
	...
	//저장매체에 저장된 인증 정보와 전달받은 인증정보를 비교 - 인증처리
	...
	//인증성공
	//session.setAttribute(String attributeName, Object attributeValue) : 클라이언트의 정보로 바인딩된 세션(Session)에 
